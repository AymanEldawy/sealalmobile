    ...
    <div className='mt-8'>
      <div className="flex justify-between gap-4 flex-col md:flex-row">
        <div className="md:flex-[2]">
          <h3 className='text-lg text-secondary font-medium mb-4'>Apple iPhone 11 64 GB Red Cell Phone Without Accessory Box (Apple Turkey Guaranteed)</h3>
          <ul className='list-disc text-[#3D3D3D]'>
            <li>
      ...
      <h3 className='text-lg text-secondary font-medium mb-4 capitalize mt-12'>{fetchWord('features', lang)}</h3>
      <div className="flex justify-between gap-4 flex-col md:flex-row mb-12">
        <table className='flex-1 border-collapse'>
          <tbody>
            {phoneChart?.map((item, index) => (
              <tr key={index} className={index % 2 !== 0 ? "" : "bg-[#ECECEC]"}>
                <td className="text-sm font-medium px-4 py-2">{item?.name}</td>
                <td className="text-sm font-medium px-4 py-2 ltr:text-right rtl:text-left">{item?.value}</td>
              </tr>
            ))}
          </tbody>
        </table>
