...
export const ProductCard = ({hideReview, grid, size, product }) => {
    ...
    <div className={`relative ${grid ? 'flex gap-4' : ' border border-gray-100 pt-2 px-2'} `}>
      ...
      <div className={grid ? 'flex-1' : ''}>
        ...
        <div className='flex gap-2 mt-2'>
          {
            product?.status ?
              <span className={`h-6 ${size === 'large' ? 'text-sm' : "text-[10px]"} bg-[#3BB77E] text-white text-xs p-1 rounded`}>
                New
              </span>
              : null
          }
          {
            product?.discount ?
              <span className={`h-6 ${size === 'large' ? 'text-sm' : "text-[10px]"} bg-[#FF541F] text-white p-1 rounded`}>
                -${product?.discount}
              </span> : null
          }
        </div>
        {
          router?.pathname === '/cart' ? (
            <Button onClick={() => {/* remove from cart  */ }} outline classes={`flex gap-1 rounded-xl !text-xs mt-3 font-semibold ${grid ? "" : 'mx-auto'} w-fit block px-2 text-sm`}>
              <DeleteIcon className="h-4 w-4" />
              {fetchWord('remove_from_cart', lang)}
            </Button>
          ) : (
            <Button onClick={() => {/* add to cart  */ }} outline classes={`flex gap-1 rounded-xl !text-xs mt-3 font-semibold ${grid ? "" : 'mx-auto'} w-fit block px-2 text-sm`}>
